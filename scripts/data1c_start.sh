#!/usr/bin/env bash
# DATA-1C VPS-first start helper.
# Create-only public OKX BTC-USDT-SWAP capture. No secrets. Never resumes a run_id.
# Prepare-only: do not start a multi-day retain until CoS assigns the window.
...
REPO_ROOT="${REPO_ROOT:-$(data1c_default_repo_root)}"
ARTIFACT_ROOT="${ARTIFACT_ROOT:-$(data1c_default_artifact_root)}"
...
if [[ -z "${DURATION_SECONDS}" ]]; then
  ...
  echo "Do not start a multi-day DATA-1C retain until CoS assigns the window after Phase A." >&2
  exit 1
fi
data1c_require_duration "${DURATION_SECONDS}"

if [[ -z "${RUN_ID}" ]]; then
  RUN_ID="$(date -u +%Y%m%dt%H%M%Sz)-live-retained"
fi
data1c_require_run_id "${RUN_ID}"

RUN_DIR="$(data1c_run_dir "${ARTIFACT_ROOT}" "${RUN_ID}")"
if [[ -e "${RUN_DIR}" ]]; then
  echo "Refuse to start: DATA-1C run directory already exists (create-only, no resume): ${RUN_DIR}" >&2
  exit 1
fi
...
tmux_state="$(data1c_tmux_alive "${TMUX_SESSION}")"
...
echo "hl_capture_tmux_alive=$(data1c_tmux_alive "$(data1c_protected_hl_session)")"
echo "signing=false"
echo "credentialless=true"
echo "twenty_four_seven=false"
echo "prepare_only=true"
echo "do_not_start_until_cos_assign=true"
...
tmux new-session -d -s "${TMUX_SESSION}" \
  "cd $(printf '%q' "${REPO_ROOT}") && unset TRADING_MODE D41_EXECUTION_MODE && export PYTHONPATH=src && exec ${BANDWIDTH_WRAP} uv run --frozen python -m hyperliquid_bot.okx_public_research --artifact-root $(printf '%q' "${ARTIFACT_ROOT}") --run-id $(printf '%q' "${RUN_ID}") --duration-seconds $(printf '%q' "${DURATION_SECONDS}")"
