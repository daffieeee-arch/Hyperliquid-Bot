#!/usr/bin/env bash
# DATA-1C operator status: tmux liveness and published Parquet part count.
...
RUN_ID="${RUN_ID:-}"
RUN_ID_SOURCE=""
RUN_PARENT="${ARTIFACT_ROOT}/data-1c/okx/BTC-USDT-SWAP"
...
if [[ -n "${RUN_ID}" ]]; then
  ...
  mapfile -t RUN_IDS < <(data1c_list_run_ids "${ARTIFACT_ROOT}")
  ...
  if [[ -n "${LIVE_ID}" ]]; then
    ...
    echo "No DATA-1C run directories under ${RUN_PARENT}/" >&2
    echo "Set RUN_ID to inspect a specific run." >&2
    RUN_ID=""
  else
    echo "Multiple DATA-1C runs found; none are a live retain. Set RUN_ID to select one:" >&2
    printf '%s\n' "${RUN_IDS[@]}" >&2
    echo "artifact_root=${ARTIFACT_ROOT}"
    echo "tmux_session=${TMUX_SESSION}"
    echo "tmux_alive=$(data1c_tmux_alive "${TMUX_SESSION}")"
...
echo "parquet_parts=$(data1c_parquet_part_count "${RAW_DIR}")"
