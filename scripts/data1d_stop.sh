...
if ! command -v tmux >/dev/null 2>&1; then
  ...
  echo "Never send C-c to hl-capture, bn-capture, bv-capture, or kr-capture from this helper." >&2
  exit 1
fi
...
echo "resume_policy=never resume or overwrite an existing DATA-1D run directory"
