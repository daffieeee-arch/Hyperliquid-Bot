...
def _ms(offset_s: float) -> int:
    ...
    return BASE_NS // 1_000_000 + round(offset_s * 1000)


def _iso(offset_s: float) -> str:
    """Event time as the microsecond ISO string Kraken markers carry."""
    stamp = datetime.fromtimestamp(_ms(offset_s) / 1000, tz=UTC)
    return stamp.strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z"


def _run_id(offset_s: int) -> str:
    stamp = datetime.fromtimestamp(BASE_NS // 1_000_000_000 + offset_s, tz=UTC)
...
def _marker(
    venue: str, product: str, channel: str, ordinal: int, payload: object
) -> RawResearchRecord:
    return _record(
        venue,
        product,
...
def _claim(
    run_dir: Path, schema: str, path_contract: str, run_id: str, venue: str, product: str
) -> None:
        ...
        _record(
            ...
            2,
            {
                ...
                "data": {
                    ...
                    "time": _ms(2),
                    "bbo": [
                        {"px": "109498.0", "sz": "1.2", "n": 3},
                        {"px": "109501.0", "sz": "0.8", "n": 2},
                    ],
                },
            },
        ),
        ...
        _record(
            ...
            5,
            {
                ...
                "data": {
                    ...
                    "time": _ms(5),
                    "bbo": [
                        {"px": "109509.0", "sz": "0.9", "n": 1},
                        {"px": "109511.0", "sz": "1.1", "n": 4},
                    ],
                },
            },
        ),
...
def binance() -> None:
    ...
    _claim(
        run_dir,
        "data-1f-retained-capture-claim-v1",
    ...
    part1 = [
        _record(
            "binance", spot, "btcusdt@trade", 1, {"e": "trade", "p": "109480.10", "q": "0.002"}
        ),
        _marker(
            "binance",
            spot,
            ...
            2,
            {
                ...
                "source_channel": "btcusdt@trade",
                "symbol": "BTCUSDT",
                "trade_id": "901",
                "price": "109480.10",
                "quantity": "0.00200",
                "exchange_event_time": str(_ms(1)),
                "trade_time": str(_ms(1)),
                "timestamp_unit": "ms",
        ...
        _record(
            ...
            "btcusdt@bookTicker",
            3,
            {"u": 5, "s": "BTCUSDT", "b": "109479.90", "B": "2.1", "a": "109480.20", "A": "1.4"},
        ),
        _marker(
            "binance",
            spot,
            ...
            4,
            {
                ...
                "source_channel": "btcusdt@bookTicker",
                "symbol": "BTCUSDT",
                "update_id": "5",
                "bid_price": "109479.90",
                "bid_quantity": "2.10000",
                "ask_price": "109480.20",
                "ask_quantity": "1.40000",
            },
        ),
        _record(
            "binance",
            ...
            5,
            {"e": "markPriceUpdate", "p": "109520.00", "i": "109500.00", "r": "0.0001"},
        ),
        _marker(
            "binance",
            usdm,
            ...
            6,
            {
                "raw_message_ordinal": 5,
                "source_channel": "usdm_mark_price",
                "context_type": "mark_price",
                "symbol": "BTCUSDT",
                "mark_price": "109520.00",
                "index_price": "109500.00",
                "funding_rate": "0.00010000",
                "event_time": str(_ms(6)),
            },
        ),
    ]
    _write_part(run_dir / "raw", 1, part1)
...
def bitvavo() -> None:
    ...
    _claim(
        run_dir,
        "data-1e-retained-capture-claim-v1",
    ...
    part1 = [
        _record(
            ...
            "trades",
            1,
            {"event": "trade", "market": "BTC-EUR", "price": "93810.5", "amount": "0.01"},
        ),
        _marker(
            "bitvavo",
            product,
        ...
        _record(
            ...
            "ticker",
            3,
            {"event": "ticker", "market": "BTC-EUR", "bestBid": "93800.1", "bestAsk": "93812.4"},
        ),
        _marker(
            "bitvavo",
            product,
        ...
        _record(
            ...
            "ticker",
            5,
            {"event": "ticker", "market": "BTC-EUR", "bestAsk": "93811.0"},
        ),
        _marker(
            "bitvavo",
            product,
...
def kraken() -> None:
    ...
    _claim(
        run_dir,
        "data-1b-retained-capture-claim-v1",
        ...
        _record(
            ...
            last_ordinal + 2,
            {
                ...
                "data": {
                    ...
                    "time": _ms(offset + 0.5),
                    "bbo": [
                        {"px": f"{price - 1:.1f}", "sz": "0.7", "n": 1},
                        {"px": f"{price + 1:.1f}", "sz": "0.6", "n": 2},
                    ],
                },
            },
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        help="artifact root to write (default: the committed fixture)",
    )
    parser.add_argument(
        "--base-utc",
        default="2026-09-05T18:00:00Z",
        help="run start; 'now' for a simulated live retain",
    )
    parser.add_argument(
        "--append-part", action="store_true", help="only append one published HL part to --out"
    )
