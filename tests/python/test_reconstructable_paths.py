...
def test_data1c_path_contract_is_stable() -> None:
    root = Path("/var/reconstructable")
    paths = data1c_run_paths(root, "sample-run")
    assert paths.contract_id == DATA1C_PATH_CONTRACT_ID
    assert DATA1C_PATH_CONTRACT_ID == "data-1c-okx-btc-usdt-swap-v1"
    assert paths.run_dir == root / "data-1c" / "okx" / "BTC-USDT-SWAP" / "sample-run"
    assert paths.raw_dir == paths.run_dir / "raw"
    assert paths.database_path == paths.run_dir / "research.duckdb"
    assert paths.capture_claim_path == paths.run_dir / "capture-claim.json"
    assert paths.capture_health_path == paths.run_dir / "capture-health.json"
    assert "<artifact-root>/data-1c/okx/BTC-USDT-SWAP/<run_id>/" in DATA1C_PATH_CONTRACT
